        ...
        <section className='min-h-screen w-full flex flex-col items-center px-4 pt-20 pb-10 sm:px-6 md:px-8'>
            ...
            {!isLoading && !isError && results.length > 0 && (
                <div className='w-full mx-auto'>
                    <section className='grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl-only:grid-cols-4 xl:grid-cols-5 gap-x-4 sm:gap-x-6 md:gap-x-8 lg:gap-x-10 xl:gap-x-12 gap-y-10 justify-items-center'>
                        {results.map((movie) => (
                            <Card
                                key={movie.id}
                                id={movie.id}
